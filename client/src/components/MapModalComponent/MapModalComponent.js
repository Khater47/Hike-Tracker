...
export default function MapModalComponent(props) {
	...
	const { markers, gpxPoints } = useMemo(() => {
		...
		if (hikePointsInfo && Object.keys(hikePointsInfo).length > 0) {
			markers =
				hikePointsInfo.hikePoints?.sort((point1, point2) => {
	...
	return (
		<Modal show={props.show} size="lg" aria-labelledby="contained-modal-title-vcenter" centered>
			<Modal.Header
				...
				closeButton
				onClick={() => props.setShowMapModal(false)}
			>
				<Modal.Title id="contained-modal-title-vcenter">Map</Modal.Title>
			</Modal.Header>
			<Modal.Body>
				{(markers?.length > 0 || gpxPoints?.length > 0) && (
					<Map markers={markers} gpxPoints={gpxPoints} />
				)}
			</Modal.Body>
		</Modal>
	);
